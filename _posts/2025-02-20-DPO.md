...
2. Reward Modelling Phase
   ...
   - 레이블링 작업자는 입력-답변 쌍들에 대해 선호도 $y_{w}\succ y_{l}\enspace|\enspace x$ 를 레이블링 함 (순서 매기기; $y_w$는 $y$ win, $y_l$은 $y$ lose)
...
-
  ...
  - $p^{*}$로 부터 샘플링된 데이터셋을 $D=\{x^{(i)}, y^{(i)}_{w}, y^{(i)}_{l}\}^{N}_{i=1}$, $\phi$로 파라미터라이징된 모델을 $r_{\phi}(x, y)$라 정의하면 선호도 최적화는 다음과 같음과 같은 NLL loss로 표현 가능하다
$$L_{R}(r_{\phi}, D) = - \mathbb{E}_{(x, y_{w}, y_{l})\sim D} [\log \sigma(r_{\phi}(x,y_{w})-r_{\phi}(x,y_{l}))]$$
  ...
  - $r_{\phi}(x, y)$는 보통 SFT 모델 $\pi^{SFT}(y|x)$로 초기화 하고, linear layer를 마지막 레이어 뒤에 추가함 (선호도 분류 목적)
   - 리워드 함수(모델) $r_{\phi}$의 variance를 낮추기 위해, 선행 연구들은 $\mathbb{E}_{x,y \sim D}[r_{\phi}(x,y)] = 0 \enspace \text{for all} \enspace x$로 설정함 (zero mean vector)

1. RL Fine-Tuning Phase
...
$$L_{DPO}(\pi_{\theta};\pi_{ref}) = -\mathbb{E}_{(x,y_{w}, y_{l})\sim D}[\log \sigma(\beta\log\frac{\pi^{*}(y_w|x)}{\pi^{ref}(y_w|x)} - \beta\log\frac{\pi^{*}(y_l|x)}{\pi^{ref}(y_l|x)})]$$

- 위 수식이 DPO의 objective이다.
- **DPO의 의의는 미분 불가능한 RLHF의 objective를 수식 정리를 통해 미분 가능하게 만들었다는 것이다.
  - implicit하게 reward 학습**
