...
기존 LLM은 토크나이저를 활용한 토큰화 과정을 제외하고는 End-to-End 방식으로 학습됩니다. 이로 인해 토큰화 과정에서 **편향**, **Domain/Modality 민감성**, **입력 문자열에 대한 민감성**과 같은 문제가 발생합니다.  그렇다고 바이트(문자) 단위로 모델을 학습하는 것은 어텐션 계산량이($O(N^2)$)이 기하급수적으로 늘어나기 때문에 실질적으로 힘들었습니다.
...
풀링된 패치 임베딩은 Linear Projection Maxtrix $\mathcal{E}_{C} \in R^{h_{\epsilon} \times (h_{\epsilon} \times U_{\epsilon})}$ 을 통해 사영됩니다. 여기서 $U_{\epsilon}$은 어텐션 헤드의 수입니다.
